function BK_execute_SPM_batch_layers_main(tmppath,subjpath,structpath,DO_COMPCOR,run,multi_regfile)
...
inputs = cell(0, 1);
spm('defaults', 'FMRI');
spm_jobman('run', batch_to_execute(stats_path,scans,timing_info_file,structpath,DO_COMPCOR,multi_regfile), inputs{:});
end

function matlabbatch = batch_to_execute(stats_path,scans,timing_info,structpath,DO_COMPCOR,multi_regfile)
...
trial_types = sort(unique(timing_info.trial_type));
...
for ii = 1:length(trial_types)
    ...
    matlabbatch{1}.spm.tools.rwls.fmri_rwls_spec.sess(1).cond(ii).name = trial_type;
    matlabbatch{1}.spm.tools.rwls.fmri_rwls_spec.sess(1).cond(ii).onset = onset;
    matlabbatch{1}.spm.tools.rwls.fmri_rwls_spec.sess(1).cond(ii).duration = duration;
    matlabbatch{1}.spm.tools.rwls.fmri_rwls_spec.sess(1).cond(ii).tmod = 0;
    matlabbatch{1}.spm.tools.rwls.fmri_rwls_spec.sess(1).cond(ii).pmod = struct('name', {}, 'param', {}, 'poly', {});
    matlabbatch{1}.spm.tools.rwls.fmri_rwls_spec.sess(1).cond(ii).orth = 1;
...
matlabbatch{3}.spm.stats.con.consess{3}.tcon.weights = [0 0 1 -1 -1 1 0];
