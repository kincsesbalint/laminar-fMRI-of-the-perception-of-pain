...
% 7376 - the design matrix is not the same across runs (second run was stopped earlier, and the rwls estimation did not work, BUT classical spm estimation work just fine) 
%
%7453
...
structpath = [structpath_base '/' num2str(subject{:}) '/ses-01/anat/presurf_MPRAGEise/presurf_UNI'];
    ...
    for run = 1:3%runs
        ...
        %smoothing kernel set to 0--> skip smoothing step
...
%         BK_execute_SPM_batch_layers_main_allrun(pipepath,subjpath,structpath,all_multireg_file.(typofcompcor),typofcompcor,hpfiltcutoff);
        BK_execute_SPM_batch_UNsmoothedmaindata(pipepath,subjpath,structpath,all_multireg_file.(typofcompcor),typofcompcor,hpfiltcutoff);
    ...
    for cmpcor=compcor
        ...
        datapath = dir([pipepath '/' typofcompcor '_UNsmoothed/*.nii']);
        outpath=[subjpath '\ses-02\func\layers\' typofcompcor '_UNsmoothed_hpf180'];
    ...
    %             toc
       movefile([pipepath '/' typofcompcor '_UNsmoothed/SPM.mat'],[outpath '/SPM.mat'])
       
       rmdir([pipepath '/' typofcompcor '_UNsmoothed'],'s')
